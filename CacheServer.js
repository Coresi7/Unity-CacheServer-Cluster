...
var maxCacheSize = 1024 * 1024 * 1024 * 500; // 50Go
...
{
    // Cache server do not free space anymore.
    return;
...
{
    // Cache server do not handle size maintanance anymore. Nor remove redundant caches.
    return 0;
    var size = 0;
...
{
    // We do not read and maintanance the total cache size.
    // Just read until it's too big for us to clear some.
    return;
    if (bytes != 0)
    {
 ...
 */
exports.StartSingle = function (a_cacheSize, a_port, a_path, a_logFn, a_errCallback)
{
    if (a_logFn)
    {
        log = a_logFn;
